...
| `analysis/` | **Замеры по реальным данным** — статистика, снятая инструментом, и отчёт по ней. Заведена нарядом `0033` (разведка по журналу отклонений, `QMS-027`). Пишет сюда **Claude Code** — одна папка, один писатель. Содержит только **агрегаты и отобранные примеры**: сами массивы данных в репо не попадают (`model/Import-Workflow.md` §Determinism / privacy). Пара на замер: `NNNN-*.md` человеку · `NNNN-*.json` машине |
...
- **Зеркало (волт):** `50_MIS-QMS/CONCEPT_mirror_EN.md` — плоская склейка всего
  `model/`, генерируется `tools/build_mirror.py`. Frontmatter `role: concept-mirror`,
  `source_version` = rev канона. **Имя файла с поколением модели не связано** (Q-10,
  2026-09-06): rev — производное поле в шапке, поэтому смена поколения файл не
  переименовывает. Русское зеркало (`…_RU.md`) — **замороженный** снимок
...
- **Guard — по содержимому, а не по ручному полю (Q-09).** Генератор штампует в зеркало
  ...
  вошедших файлов и вердикт, **ничего не пишет**. Вердиктов четыре, в порядке «сначала
  целость, потом свежесть» (QMS-019): `OK` 0 · `CORRUPT` 3 · `STALE` 1 · `UNVERIFIED` 1 —
  второй штамп `body_hash` считается по телу зеркала и отвечает на вопрос «тот ли это
  файл», тогда как `source_hash` — на «из какого поколения канона». Прежний сторож сверял
...
python tools/build_mirror.py --check build/mirror/CONCEPT_mirror_EN.md
```
