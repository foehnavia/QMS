...
# --- Эталонные строки жёстких форматов (наряд `0034` §2, класс «жёсткий формат») -
#
# Ширина колонки такого класса выводится **замером этой строки** тем же шрифтом,
# которым колонку нарисуют, — не числом знакомест с запасом в четверть. Длины не
# придуманы: они сняты с журнала отклонений за 2025 год
# (`docs/analysis/0033-import-journal-stats.json`, 1116 строк, разведка наряда
# `0033`), поэтому эталон — не «примерно такой», а самый длинный из встреченных.
#
# Живут здесь, а не в `kit`: формат бизнес-номера — знание предметной области,
# а `kit` про оформление. Экран берёт эталон отсюда и отдаёт в `kit.fixed`.

#: Номер детали (`מק"ט`): максимум журнала — **13** знаков (медиана 9, p95 10).
#: Прежняя сетка мерилась по `MF5-10375A-N` (12) и на тринадцатом резала (§5).
SAMPLE_ITEM_NUMBER = "MF5-10375A-NX"

#: Наряд (`פק"ע`): максимум журнала — **10** знаков (медиана и p95 — 9).
#: Прежняя сетка мерилась по `W26007336` (9), и десятый знак не помещался (§5).
SAMPLE_WO = "W260073361"

#: Бизнес-номер отклонения — формат наш, а не цеховой: `DEV-YYMMDD-NNNN`.
SAMPLE_DEVIATION = "DEV-260903-0001"

#: Дата в полном формате колонки.
SAMPLE_DATE = "09.09.2026"
...
#: **Закрытый список** подписей колонки исхода — всё, что в ней может стоять.
#: Ширина колонки берётся замером самой длинной из них (`kit.pill`), а не числом
#: знакомест: исход — значение контролируемого словаря, и оценивать тут нечего
#: (наряд `0034` §2). Прочерк «решения ещё нет» входит наравне с принятыми: в
#: колонке он стоит так же часто, как они.
DECISION_DEV_COLUMN = (*DECISION_DEV_SHORT.values(), NO_DECISION_SHORT)
