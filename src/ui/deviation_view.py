...
#: `Explanation` — остаток: 1216 − 958 = 258, и это выше её пола по заголовку (94).
...
class DeviationView(QWidget):
    ...
    def _revision_width(self) -> int:
        ...
        сетка была нарисована. Правило §1 наряда — «по заголовку».

        Считается **замером плюс непечатаемое**, а не через `kit.FIT_LABEL`:
        та формула даёт «заголовок + отступы ячейки» и не знает про линию сетки
        и собственные поля Qt, поэтому её 68 px оставляли тексту 41 при нужных
        48 — заголовок резался при идеально сходившейся сумме (`CLAUDE.md`
        §9а.12). Саму формулу здесь не чиню: это `kit`, поведение всех экранов
        и отдельная задача QMS-022.
        """
        return self.table.fontMetrics().horizontalAdvance("Revision") + self._cell_chrome()

    def _cell_chrome(self) -> int:
        """Сколько пикселей ячейки не достаётся тексту — **замером**.

        Линия сетки, отступы листа стиля и собственные поля Qt. На этой
        платформе 27; зашитое число врало бы на другой молча.
        """
        from PySide6.QtWidgets import QStyle, QStyleOptionViewItem  # noqa: PLC0415

        margin = self.table.style().pixelMetric(
            QStyle.PixelMetric.PM_FocusFrameHMargin, QStyleOptionViewItem(), self.table
        )
        return 1 + tokens.PAD_CELL * 2 + 2 * (margin + 1)
