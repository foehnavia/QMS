...
#: Полная сетка уровня отклонения — **пиксели, назначенные замером**.
#:
#: Числа канвы были отправной точкой; норма — замер на нативной платформе
#: (`CLAUDE.md` §9а.12, `design-system.md` §3 rev. 1.8: «A declared column width
#: is not a drawn column width»). Между объявленным числом и текстом стоит
#: **27 px непечатаемого**: линия сетки 1, отступы листа стиля 20 и собственные
#: поля Qt 6. Поэтому здесь всюду «нужно тексту + 27», и ни одно число не взято
#: на глаз.
#:
#: `WO` 92 → 100 (доводка, пункт 3): `W26007336` требует 68 + 27 = 95, и прежние
#: 92 резали его до `W260073…` при идеально сходившейся сумме.
#: `Findings` 280 → 303: самая широкая реальная пилюля —
#: `Dim. 18 · − 0.05 · Cutting-edge width` — требует 282 плюс оправа ячейки 21.
#: `Revision` считается по заголовку: её в канве нет, колонка появилась нарядом `0025`.
FULL_WIDTHS = {
    EXPANDER: 34,
    "Number": 200,
    "Item": 123,
...
#: Сетка при минимальном окне — сумма ровно **1216** (доводка, пункт 2).
#:
#: `Findings` и `Explanation` здесь **остаются** (доводка Д-1, решение
#: пользователя). Прежний порядок снимал `Findings`, обосновывая это тем, что
#: находки на этой ширине показаны строками, — но строками они показаны только у
#: **раскрытой** записи, а свёрнутая при 1280 про находки не говорила ничего.
#: То есть требование, ради которого экран переделан, при минимальном окне не
#: выполнялось вовсе.
#:
#: Каждое число — замер плюс 27 px непечатаемого:
#:   `Number`    104 (15 знаков полужирным) → 131
#:   `Item`       88 (`MF5-10375A-N`)       → 115
#:   `WO`         68 (`W26007336`)          →  95, взято 100 (пункт 3)
#:   `Date`       48 (короткий `07.09.26`)  →  75, взято 76
#:   `Qty`        28 (`9999`; заголовок 20) →  55, взято 56
#:   `Decision`  116 (пилюля «Not decided») → 143
#:   `Findings`  282 (пилюля) + оправа 21   → 303
#: **Две правки из фактических данных (наряд `0034` §5).** Обе сетки мерились по
#: значениям, которые журнал за 2025 год перекрывает
#: (`docs/analysis/0033-import-journal-stats.json`, 1116 строк):
#:   `Item` — эталон был `MF5-10375A-N`, двенадцать знаков; максимум `מק"ט` —
#:     **тринадцать**. Замер тринадцатизначного: 96 + 27 = **123** (было 115/120).
#:   `WO`   — эталон был `W26007336`, девять знаков; максимум `פק"ע` — **десять**.
#:     Замер десятизначного: 75 + 27 = **102** (было 100 в обеих сетках).
#: Обе добавки берёт на себя `Explanation`, поэтому объявленные суммы не меняются:
#: `TIGHT` остаётся ровно 1216, `FULL` — 1517.
#: `Explanation` — остаток: 1216 − 968 = 248, и это выше её пола по заголовку (94).
#: Прежний «минимум 367» вышел из завышающей формулы и нормой не является.
TIGHT_WIDTHS = {
    EXPANDER: 34,
    "Number": 131,
    "Item": 123,
...
#: Пол `Explanation` — её собственный заголовок: колонка свободного текста
#: обрезается по устройству, и полный текст живёт в подсказке. Ниже заголовка
#: она опуститься не может — колонка, чью подпись не прочесть, не колонка.
EXPLANATION_FLOOR = 94

#: Что уходит при сжатии — **одна** версия и ровно эта (доводка, пункт 1):
#: зона запаса и `Revision`; `Date` переходит в короткий формат. `Findings` и
#: `Explanation` остаются: без первой свёрнутая запись при 1280 нема, без второй
...
#: Полная формулировка заголовка `Qty` — в подсказке (канва, §1 наряда).
QTY_HINT = "parts in this deviation"
...
def grid_widths(window_width: int, *, revision_width: int) -> dict[str, int]:
    ...
    Сторожит правило доводки `0028`, пункт 2: «ширины уровня отклонения при 1280
    назначить **замером**, целевая сумма — **1216**», и пункт 1: «`Findings` и
    `Explanation` **остаются** при 1280; уходят зона запаса, `Revision`; `Date`
    переходит в короткий формат».

    Предел берётся у `kit.table_limit` — правило про таблицы вообще
    (`design-system.md` §3 rev. 1.8: не шире 90 % окна и не уже 1216).

    Ширина `Revision` передаётся, а не берётся из таблицы, потому что она
    единственная считается шрифтом («по заголовку»): при 1280 колонка уходит, и
    на сумму 1216 её значение не влияет никак — это и есть причина, по которой
    сумма проверяема без шрифта.
    """
    limit = kit.table_limit(window_width)

    full = dict(FULL_WIDTHS)
    full["Revision"] = revision_width
    if sum(full.values()) <= limit:
        return full

    # Сжатая сетка — не «полная минус что-нибудь», а **своя**, назначенная
    # замером целиком. Снимать по колонке из полной значило бы получать разные
    # числа при разных ширинах окна и мерить каждое из них отдельно.
    tight = dict(TIGHT_WIDTHS)
...
class DeviationView(QWidget):
    ...
    def __init__(self, engine: Engine, parent: QWidget | None = None) -> None:
        ...
        # Заголовок сокращён до `Qty` (доводка, пункт 4): прежний `Dev. qty` был
        # шире содержимого и один задавал ширину колонки. Полная формулировка не
        # исчезает — она переезжает в подсказку заголовка, как и требует канва.
        self.table.horizontalHeaderItem(COLUMNS.index("Qty")).setToolTip(QTY_HINT)

        # Обоснование — **двухстрочное**, и только оно (канва §1, критерий 3
        # доводки). Общее правило `kit` — ячейка однострочная, длинное режется с
        # подсказкой; здесь оно отменяется для одного экрана осознанно: в одну
        # строку реальное обоснование не влезает ни при какой ширине, а это
        # главный текст прецедента. Прочие колонки от переноса не меняются —
        # их значения короче строки.
    ...
    def _revision_width(self) -> int:
        ...
        сетка была нарисована. Правило §1 наряда — «по заголовку», и считает его
        теперь `kit.column_width` с `FIT_LABEL`, а не своя формула на этом экране.

        **Здесь стояла вторая формула непечатаемого, и она врала.** Она
        прибавляла `1 + PAD_CELL * 2 + 2 * (PM_FocusFrameHMargin + 1)` = 27 px, а
        стиль под листом канона оставляет ячейке 21 (заголовку — 20): член
        `2 * (margin + 1)` до отрисовки не доходит вовсе. Замер приложен к отчёту
        наряда `0034`; из-за этих шести пикселей колонка получала 75 вместо
        нужных 68, а `card_dialog` записал в комментарий «недобор 7 px, чинится
        QMS-022» — недобора не было. Двух формул одной величины больше нет:
        считает `kit.metrics`, и только он (§1 наряда `0034`).
        """
        return kit.column_width(self.table, kit.FIT_LABEL, "Revision")
    ...
    def _date_text(self, value) -> str:
        """Короткий формат при сжатии (доводка, пункт 1).

        Признак — уход `Revision`: она единственная колонка, которая при сжатии
        снимается, и «сетка сжата» это ровно её отсутствие. Прежний признак
        (уход `Findings`) перестал существовать вместе с самим уходом.
        """
        compact = "Revision" not in grid_widths(
