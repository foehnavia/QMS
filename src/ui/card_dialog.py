...
#: **Сжатая** сетка `DIALOG_FULL` — назначена **заново целиком** нарядом `0038`
#: §0, а не подправлена. Вопрос стоял «как разложить полотно», а не «кто отдаст
#: 37 px»: до этого сетка держала `Item` 92 и `WO` 96 при замеренном пределе
#: предметной области 123 и 102, и тринадцатизначный `מק"ט` при 1180 обрезался.
#:
#: **Объявленный порядок, кто платит обрезкой** — это и есть содержание правки:
#:
#:   1. **идентификаторы не обрезаются никогда** — `Deviation`, `Item`, `WO`,
#:      `Date`, `Revision`. Ширина по замеренному пределу **предметной области**,
#:      а не по тому, что оказалось в базе прогона: обрезанный `מק"ט` — запись,
#:      которую нельзя опознать;
#:   2. **составная подпись** `Characteristic` обрезается **второй**, хвост в
#:      подсказку: она композит, и по началу строки узнаётся;
#:   3. **свободный текст** `Explanation` платит **первым**, до своего пола по
#:      заголовку: это запись, которую дочитывают в подсказке.
#:
#: Модель классов наряда `0034` их не уравнивает, и прежняя формулировка «на
#: минимуме давать нечего» обращалась со всеми колонками как с равными
#: претендентами. У идентификатора ширина есть **свойство формата**, у свободного
#: текста — **остаток**.
#:
#: Раскладка (замер на нативной, полотно **1126** — с учётом вертикальной полосы,
#: которая поднимается при раскрытии строк; без неё полотно 1136):
#:
#:   (раскрыватель)     30   §3 наряда `0031`, не трогается
#:   Deviation         132   `DEV-260909-0001` просит 131
#:   Item              123   максимум `מק"ט` по журналу 13 знаков (§5 `0034`)
#:   Date               92   `19.08.2026` просит 89
#:   Revision       заголовок `FIT_LABEL`, 68
#:   WO                102   максимум `פק"ע` по журналу 10 знаков (§5 `0034`)
#:   Sign · value       96   заголовок 86, значения короче
#:   Decision       пилюля    оправа пилюли, не текст ячейки
#:   Insp.          заголовок `FIT_LABEL`, 47
#:   ---------------------------- непокупаемое: 827
#:   Characteristic    195   **потребность целиком** — перестала обрезаться
#:   Explanation       104   остаток; пол по заголовку 87, запас 17
#:   ---------------------------- сумма: **1126**
#:
#: Что это стоило: `Explanation` 196 → 104. Обмен объявлен и сделан сознательно —
#: обоснование прецедента читается в подсказке, номер детали не читается нигде.
...
#: Почему у размера нет канона — **подсказкой групповой строки**, и только ею.
#:
#: Заголовок `Search by canonical position is unavailable` в 14 pt отсюда снят
#: (§1 наряда `0038`): он говорил третий раз то, что уже сказано колонкой `Canon`
#: со значением `not bound` и знаком `!` на самом размере, занимал 203 px в
#: единственной области, ради которой карточку открывают, — и **в массовом
#: случае был неверен по существу**. Массовый случай это не-CG размер, у которого
#: канона нет **по природе**; предлагать там привязку значит предлагать действие,
...
class CardDialog(QDialog):
    ...
    def __init__(
        ...
        # **Привязки нет — область не показывает ничего** (§1 наряда `0038`).
        # Здесь стояло пустое состояние с заголовком, объяснением и второй
        # кнопкой `Mapping…`; снято целиком, и взамен не поставлено ничего —
        # ни короткой строки, ни сжатого блока.
        #
        # Достаточным сообщением остаются групповая строка с нулём («By canon:
        # this characteristic is not bound (0)», с этим же объяснением в
        # подсказке) и счётчик на вкладке. Пустой L1b есть **ожидаемый
        # результат**, а не тупик, из которого нужен выход.
        #
        # Действие не теряется: кнопка `Mapping…` под таблицей находок остаётся и
        # есть единственное его место — та же `bind_canon`, что звалась отсюда.
        #
