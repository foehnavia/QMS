...
# Тестовый режим `show_error` — один раз на сессию, **явным** флагом (наряд 0027 §1).
#
# Без него экран, позвавший `show_error` там, где тест ждал успеха, вешал прогон:
# модальное окно под offscreen ждёт ответа вечно, и вывода нет ни строки. Отказ
# повторился в нарядах `0004`, `0019`, `0024` и каждый раз стоил трёх заходов
# диагностики вслепую. Теперь то же место бросает `UnexpectedErrorDialog`.
#
# Правило перехвата `CLAUDE.md` §9 в силе и не отменяется: тесты **пути отказа**
# подменяют `ui.kit.show_error` целиком, и подмена стоит раньше режима.
#
# Вызов на уровне модуля, а не фикстурой: фикстуру можно не запросить, а режим
# обязан действовать на весь прогон — включая тесты, которые о нём не знают.
from ui.kit.widgets import set_test_mode  # noqa: E402  (после QT_QPA_PLATFORM выше)
