...
def test_missing_characteristic_is_created_without_form_or_mapping(seeded_session: Session) -> None:
    ...
    char, created = get_or_create_characteristic(seeded_session, rev(item), "19")
    seeded_session.commit()

    assert created is True
    assert char.local_number == "19"
    assert char.revision is rev(item)
...
def test_repeated_call_returns_the_same_row(seeded_session: Session) -> None:
    ...
    first, created_first = get_or_create_characteristic(seeded_session, rev(item), "19")
    second, created_second = get_or_create_characteristic(seeded_session, rev(item), "19")
    seeded_session.commit()
...
def test_whitespace_is_trimmed_before_matching(seeded_session: Session) -> None:
    ...
    first, _ = get_or_create_characteristic(seeded_session, rev(item), "19")
    second, created = get_or_create_characteristic(seeded_session, rev(item), "  19 ")
...
def test_same_number_on_another_item_is_a_separate_row(seeded_session: Session) -> None:
    ...
    a, _ = get_or_create_characteristic(seeded_session, rev(first), "12")
    b, created = get_or_create_characteristic(seeded_session, rev(second), "12")
    seeded_session.commit()
...
def test_blank_local_number_is_rejected(seeded_session: Session) -> None:
    ...
    with pytest.raises(ValidationError):
        get_or_create_characteristic(seeded_session, rev(item), "  ")
